...
internal fun RunnerToggleButton(
    ...
    Row(
        ...
        if (phase.isBusy) {
            ...
            val isForeground = state.runMode == RunMode.FOREGROUND
            MaaButton(
                onClick = {
                    if (isForeground) {
                        onIntent(SessionIntent.ShowOverlay)
                    } else {
                        onIntent(SessionIntent.Start())
                    }
                },
                enabled = if (isForeground) true else state.canStart,
                ...
                Text(
                    if (isForeground) stringResource(R.string.settings_overlay_show)
                    else stringResource(R.string.runner_start)
                )
            }
        }
